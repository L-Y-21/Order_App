...
public async Task<(bool Success, string Message)> PrintInvoiceAsync(Order order, string printerName = "", string? operatorName = null)
    {
        ...
        private byte[] BuildEscPosReceipt(Order order, Company? company, string? operatorName)
        {
            ...
            foreach (var item in order.Items)
            {
                ...
                // Split item name - main name with qty/price/total, parenthetical info on next line
                int parenIndex = nameRaw.IndexOf('(');
                if (parenIndex > 0)
                {
                    var mainName = nameRaw.Substring(0, parenIndex).Trim();
                    var parenInfo = nameRaw.Substring(parenIndex).Trim();
                    var line = $"{mainName,-20} {item.Quantity,3:F0}  {item.UnitPriceSnapshot,6:F2}    *{item.LineTotal,9:F2}";
                    Line(line);
                    Line(parenInfo);
                }
                else
                {
                    var line = $"{nameRaw,-20} {item.Quantity,3:F0}  {item.UnitPriceSnapshot,6:F2}    *{item.LineTotal,10:F2}";
                    Line(line);
                }
            }

            Line(new string('-', LineWidth));
            Line(TwoCol($"TAXBL {order.Items.Count()}", $"*{order.Subtotal,10:N2}"));
            Line(TwoCol("TAX1 15%", $"*{order.TaxAmount,10:N2}"));
            if (order.WithholdingAmount > 0)
            {
                Line(TwoCol("WH 3%", $"*{order.WithholdingAmount,10:N2}"));
            }
            Line(new string('-', LineWidth));

            // TOTAL - big, same row (uses half-width column since chars are double-wide)
            DoubleWidthHeight();
            Line(TwoCol("TOTAL", $"*{order.TotalAmount,10:N2}", DoubleWidthLineWidth));
            NormalSize();
            Line();
            ...
            //BoldOn();
            DoubleWidthHeight1();
            Line(TwoCol("CASH#", $"*{order.TotalAmount,10:N2}", DoubleWidthLineWidth));
            NormalSize();
            BoldOff();
            Line($"ITEM #");
            Line();
